...
| [`17-fret-and-the-selection-vs-classification-insight.md`](17-fret-and-the-selection-vs-classification-insight.md) | **The decisive nonlinear test (FRET/XOR) + the key insight.** Built the strongest honest nonlinear regime (FRET + co-localization XOR; all-bands gap **+0.113**, marginal-MI oracle *fails*). Result: **AE ties pca_load (0.633 vs 0.641, within noise)** even here. Why: **selection ≠ classification** — *informative ⟹ has variance ⟹ PCA-selectable*; the nonlinearity is resolved by the downstream classifier, not the selector. So a *blind* AE selector cannot beat pca_load "by far more"; that needs labels (a discriminability selector). | Anyone who believes "AE must beat PCA on nonlinear data." |
| [`18-metric-suite.md`](18-metric-suite.md) | **The diamond-solid metric battery.** Every metric that makes sense for band selection, grouped by what it proves: (A) info-retention panel (F1/bacc/MCC/κ/gap), (B) classifier-independent info (relevance/redundancy/mRMR), (C) ground-truth band fidelity, (D) stability, (E) significance (CI/Wilcoxon/d). 8 selectors × 4 regimes × 6 seeds. Verdict: **pca_load strongest blind overall (wins clean *significantly*, best GT-precision); AE competitive — ties realistic/FRET, *significantly beats pca on nonlinear reabsorption*.** **Revises** the earlier "AE wins clean" claim (noise-dependent). | Anyone needing the rigorous, significance-tested evidence. |
| [`19-fullspectrum-vs-selection-mixnoise.md`](19-fullspectrum-vs-selection-mixnoise.md) | **Full-spectrum vs selection across 5 mix/noise levels (real-instrument regime).** High-res 564-band data + realistic fixed-pattern **clutter** + few-shot labels, so full-band data overfits. Chemicals **are** discriminative. **Supervised selection beats full-data, growing with noise (+0.007→+0.056);** blind AE **matches** full from 24/564 bands while **PCA stays below full**; **AE beats PCA under noise (L3–L5, up to +0.020).** Motivates a semi-supervised AE selector. | Anyone weighing full-spectrum vs selection on noisy data. |
